...
Without Replacement     $n(n-1)(n-2)...(n-k+1)=\frac{n!}{(n-k)!}$         $\binom{n}{k}$
...
\textbf{Without Replacement} & $n(n-1)(n-2)...(n-k+1)=\frac{n!}{(n-k)!}$ & $\binom{n}{k}$\\
...
2. What is Pr($X=5^C$)? In other words, what is the probability that the outcome will not be 5?
...
Most questions in the social sciences involve events, rather than numbers per se. To analyze and reason about events quantiatively, we need a way of mapping events to numbers. A random variable does exactly that.
...
The formal definition of a random variable is easier to given by separating out two cases: discrete random variables when the numeric summaries of the events are discrete,  and continuous random variables whern they are continuous.
...
Example: For a fair die, $E(Y)=7/2$ 
