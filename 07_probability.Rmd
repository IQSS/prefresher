...
\begin{framed}
\parbox[c]{4.55in}{Example:  For a fair die,\\ $P(Y\le 1)=$\\ $P(Y\le 3)=$\\ $P(Y\le 6)=$} % \parbox{1.5in}{\includegraphics[width=1.25in]{diecmf.eps}}
\end{framed}
...
5. $\text{Cov}(aX,bY) = ab Cov[X,Y]$
6. $\text{Cov}X+a,Y) =  Cov[X,Y]$
...
2. **Sample correlation**: $\rho = \frac{\text{Cov}(X,Y)}{\sqrt{\text{Var}(X)\text{Var}(Y)}}$


```{example}
Example:  Using the above table, calculate:
...
as $n\to \infty$. In other words, $P( \lim_{n\to\infty}\bar{X}_n = \mu) = 1$. This theorem, known as the Strong Law of Large Numbers, is an important motivation for the widespread use of the sample mean, as well as the intuition link between averages and expected values. 
