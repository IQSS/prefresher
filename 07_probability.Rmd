...
__Continuous Random Variable__:  $Y$ is a continuous random variable if there exists a non-negative function $f(y)$ defined for all real $y\in (-\infty,\infty)$, such that for any interval $A$, $\Pr(Y\in A)=\int\limits_A f(y)dy$. Examples: age, income, GNP, temperature.
...
1. $\Pr(X = 5) = \frac{4}{16}$, $\Pr(X = 3) = \frac{2}{16}$, $\Pr(X = 6) = \frac{3}{16}$

  
2. What is $\Pr(X=5 \cup X = 3)^C = \frac{10}{16}$? 
...
3. $V[X] = E[X^2] - E[X]^2 = E[Y] - E[X]^2 = \frac{5}{2} - \frac{7}{30}^2 \approx 2.45$
