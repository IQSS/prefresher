...
```{r, incldue=FALSE, message = FALSE, warning = FALSE}
library(dplyr)
library(readr)
library(haven)
library(ggplot2)
...
Some common operaters that can be used to filter or to use as a condition. Remember, you can use the unique function to look at the set of all values a variable holds in the dataset. 
```{r}
...
Use R to write code that will consecutively multiply $A$ to itself 4 times. What is the value of $A^{4}$? 
...
First, show all observations (rows) that have the "male" variable value that is higher than 0.5
...
Next, show all values of "trust_neighbors" and "age"
...
```
First, get a small subset of the nunn_full dataset. This time, sample 20 rows and select for variables exports, age, age2, male, and urban_dum. To this small subset, add (cbind) a column of 1's; this represents the intercept. If you need some guidance, look at how we sampled 10 rows selected for a different set of variables above in the lecture portion. 
```{r}
# Enter here
```
