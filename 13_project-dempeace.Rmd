---
title: "`R` application with 'The Democratic Peace'"
subtitle: "Programming Module 3 - Math Prefresher 2017"
...
```{r}
mid_year_by_year <- foreach(i = 1:nrow(mid), .combine = "bind_rows") %do% { 
  
  data_frame(ccode = mid$ccode[i], # row i's country
...
Here we can do a `left_join` from `polity` to `mid`. We want to keep the rows in `polity` that do not match in `mid`, and label them as non-disputes.
```{r}
...
```{r}
p_m <- p_m %>%
  mutate(dispute = replace(dispute, is.na(dispute), 0))
```

long to wide
```{r}
p_m_w <- dcast(data = as.data.table(p_m),
               formula = ccode ~ year,
               value.var = "polity2")
```
...
Often, files we need are saved in the `.xls` or `xlsx` format. It is possible to read these files directly into `R`, but experience suggests that this process is slower than converting them first to `.csv` format and reading them in as `.csv` files. The `rio` package^[<https://github.com/leeper/rio>] has functions to convert between file types. 

`readxl`/`readr`/`haven` packages(<https://github.com/tidyverse/tidyverse>) is constantly expanding to capture more file types. `read_xl` is new but quite good.
...
## Task 2: Data Merging
We will use data to test a version of the Democratic Peace Thesis (DPS): are democracies less likely to engage in militarized interstate disputes with one another? 
...
* Once you have merged the datasets, what fraction of all MIDs in the time period we have data for involve 2 countries both with Polity2 scores greater than 5? Greater than 8? If there are any of the latter, find out what MIDs those are and research them for a bit. 
* What is the mean difference in Polity2 score for countries involved in a MID? How has this quantity changed over time? How do you interpret this? 
