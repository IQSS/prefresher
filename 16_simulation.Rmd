...
(a) First, load `usc2010_1percent.Rds` into your R session. After loading the `library(tidyverse)`, browse it. Although this is only a 1 percent extract, treat this as your population for pedagogical purposes. What is the population proportion of non-White residents?
...
(b) Setting a seed to `1669482`, sample 100 respondents from this sample. What is the proportion of non-White residents in this _particular_ sample? By how many percentage points are you off from the true proportion?
...
(c) Now imagine what you did above was one survey. What would we get if we did 20 surveys? To simulate this, write a loop that does the same exercise 1000 times, each time computing a sample proportion. Use the same seed at the top. Show a histogram of these _20 draws_. Try doing this with a `for` loop and storing your results in a new length-20 vector. What is the average of the 20 sample estimates?
...
(d) Now, to make things more real, let's introduce some response bias. Suppose that non-White residents are 10 percent less likely to respond to enter your survey than the other two categories. This is plausible if you think that the Census is from 2010 but you are polling in 2018, and racial minorities are more mobile than Whites.  Repeat the same exercise in (c) by modeling this behavior. You can do this by creating a variable, e.g. `propensity`, that is 0.9 for renters and 1 otherwise. Then, you can refer to it in the propensity argument.
...
Read in the Upshot Siena poll from November 2016, `input/upshot-siena-polls.csv` 
