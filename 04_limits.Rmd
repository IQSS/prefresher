...
where $\mu$ is the mean of $X$ and $\sigma$ is the standard deviation of $X$. The arrow is read as  "convergens in distribution to". $\text{Normal}(0, 1)$ indicates a Normal Distribution with mean 0 and variance 1. 

That is,  the limit of the distribution of the lefthand side is the distribution of the righthand side. 
```

The sign of a limit is the arrow "$\rightarrow$". Although we have not yet covered probability (in Section \@ref(probability)) so we have not described what distributions and random variables are, it is worth forshadowing the Central Limit Theorem. The Central Limit Theorem is powerful because it gives us a _guarantee_ of what would happen if $n \rightarrow \infty$, which in this case means we collected more data.
...
In math, we need a formal construct for what it means to be "near" a point $\bf c$ in ${\bf R}^n$.  This is generally called the __neighborhood__ of $\bf c$. It's represented by an open interval, disk, or ball, depending on whether ${\bf R}^n$ is of one, two, or more dimensions, respectively.

1. Open interval: $$(a,b)\equiv \{ x\in{\bf R}^1: a<x<b\}$$

$x$ is a one-dimensional element in which x is greater than a and less than b

2. Closed interval: $$[a,b]\equiv \{ x\in{\bf R}^1: a\le x \le b\}$$

$x$ is a one-dimensional element in which x is greater or equal to than a and less than  or equal to b

3. Half open, half closed: $$(a,b]\equiv \{ x\in{\bf R}^1: a<x\le b\}$$

$x$ is a one-dimensional element in which x is greater than a and less than or equal to b


Given the point $c$, the "neighborhood" are defined as a $\epsilon$-interval in ${\bf R}^1$: $\{x : |x-c|<\epsilon \}$. $x$ is in the neighborhood of {\bf c} if it is in the open interval $(c-\epsilon,c+\epsilon)$.
