...
```{theorem, lln, name = "(Weak) Law of Large Numbers"}
...
Intuitively, the more data, the more accurate is your guess.  For example, the Figure \@ref(fig:llnsim) shows how the sample average from may coin tosses converges to the tre value : 0.5.
...
```{exercise}
Find the following limits, then explain in English the intuition for why that is the case. 
...
fx3 <- ggplot(range3, aes(x = x)) +
  stat_function(fun = function(x) 1 + 1/(x^2), size = 0.5) +
