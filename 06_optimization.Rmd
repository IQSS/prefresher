...
## Example: Meltzer-Richards {-}

A standard backdrop in comprative political economy, the Meltzer-Richards model states that redistribution of wealth should be higher in societies where the median income is much smaller than the average income. Why is that the case? Here is a simplified example that is not the exact model by Meltzer and Richard's, but adapted from Persson and Tabellini^[Adapted from Torsten Persson and Guido Tabellini, _Political Economics: Explaining Economic Policy_. MIT Press. ]
...
- $U(\cdot)$, u for "utility", is a function that is concave and increasing, and expresses the utility gained from public goods. This tells us that its derivative of is _positive_, and its second derivative is __negative__. 
...
What does this say about the level of redistribution we observe in an economy? The higher the average income is than the median income, which often (but not always) means _more_ inequality, there should be __more__ redistribution.
...
fx + fprimex + plot_annotation(expression(f(x)==x^2+2))
```
...
A function f is quasiconcave over the set $S$ if $\forall x_1,x_2 \in S$ and $\forall a \in (0,1)$, $$f(ax_1 + (1-a)x_2) \le \max(f(x_1),f(x_2))$$
...
Quasilinear: A function that is both quasiconvex and quasiconcave is quasilinear.
...
\begin{align*}
\mathbf{x}^T H(\mathbf{x}^*) \mathbf{x} &= \begin{pmatrix} x_1 & x_2 \end{pmatrix}\\
