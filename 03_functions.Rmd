...
Sometimes (often?) the inverse does not exist. For example, we're given the function $y=x^2$ (a parabola).  Solving for $x$, we get $x=\pm\sqrt{y}$. For each value of $y$, there are two values of $x$. Remember, function by definition has a single function value for each element of its domain. This means, there cannot be "one-to-many" mapping.
...
```{exercise, name = "Solving for Variable", solvevar1}
