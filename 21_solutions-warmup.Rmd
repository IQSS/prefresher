...
2. $\lim\limits_{x \to 2} \frac{(x - 2) (x - 1)}{(x - 2)} = 1 \text{ if } x = 2, \text{ undefined otherwise}$
3. $\lim\limits_{x \to 2}\frac{x^2 - 3x + 2}{x- 2} = 1 \text{ if } x = 2, \text{ undefined otherwise}$
...
1. If there are 12 cards, numbered 1 to 12, and 4 cards are chosen, $ _{12}\text{C}_4 = \frac{12\cdot 11\cdot 10\cdot 9}{4!} = 495$ possible hands exist (unordered, without replacement) .
...
4. If we roll two fair dice, what is the probability that their sum would be 12? $\leadsto \text{also } \frac{1}{18}$ 
