...
```{proof}
#| label: pwrinduct
#| name: Proof of Power Rule by Induction
...
```{exercise} 
#| label: introderivatives
#| name: Derivative of Polynomials
...
```{example}
#| name: Succession of Derivatives
\begin{align*}
f(x) &=x^3\\
f^{\prime}(x) &=3x^2\\
f^{\prime\prime}(x) &=6x \\
...
```{example}
#| name: More than one type of partial
...
```{definition}
#| name: The Definite Integral (Riemann)
...
```{theorem}
#| name: First Fundamental Theorem of Calculus
...
```{example}
#| label: defintmon
#| name: Definite Integral of a monomial
...
```{exercise}
#| name: Definite integral shortcuts
...
```{example}
#| label: intsub1
#| name: Integration by Substitution I
...
```{example}
#| label: intsub2
#| name: Integration by Substitutiton II
